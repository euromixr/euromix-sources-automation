...
async function run() {
    console.log("🚀🚀🚀 VERSION 2.0 - NEW CODE RUNNING! 🚀🚀🚀");
    let browser;
    try {
        ...
        console.log(`✅ נמצאו ${uniqueArticles.length} כתבות ייחודיות (NOT 1160!).`);
        ...
        console.log(`⏰ ${recentArticles.length} כתבות מ-24 שעות (NOT 48!).`);
...
async function aggressiveAutoScroll(page) {
    await page.evaluate(async () => {
        await new Promise((resolve) => {
            let totalHeight = 0, distance = 100, count = 0;
...
async function updateStatusTime() {
    try {
        await db.collection('artifacts').doc(APP_ID).collection('public').doc('data')
            .collection('settings').doc('status').set({ lastScrape: admin.firestore.Timestamp.now() }, { merge: true });
    } catch(e) {}
}
